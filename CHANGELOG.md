...
- LP-exact degree measures (n ≤ 12): `approximate_degree`,
  `one_sided_approximate_degree`, and `threshold_degree` now solve their
  defining linear programs exactly instead of returning heuristics, and
  `nondeterministic_degree` computes de Wolf's polynomial nondeterministic
  degree exactly via null-space rank (#119)
- A guarantee classification for every function in
  `analysis.query_complexity` (exact / certified lower bound / clamped
  estimate), documented in the module docstring and surfaced in
  `QueryComplexityProfile.summary()` (#119)
- Cross-validation of adversary bounds against pinned ADV± SDP optima from
  QuantumQueryOptimizer 0.1.4 (`tests/cross_validation/test_qqo.py`,
  fixtures generated by `scripts/generate_qqo_fixtures.py`), plus
  exhaustive guarantee tests over all 2–3 variable functions (#119)
- Exhaustive census cross-validation against OEIS (Dedekind, unate,
  canalizing, bent counts) and the He–Macauley canalizing-depth
  stratification; n ≤ 3 per PR, full n = 4 on main (#114)

### Fixed

- `is_monotone` and `is_unate` incorrectly returned `True` for every
  function; both now check all hypercube edges vectorized (#114)
- `ambainis_complexity` and `spectral_adversary_bound` were unseeded
  sampling heuristics that could exceed the true adversary bound; they are
  now deterministic, certified lower bounds on ADV± (exact values of the
  sensitive-edge witness), and `general_adversary_bound` is their maximum
  (#119)
- `nondeterministic_complexity` returned the minimum certificate over side
  inputs; it now returns C_side(f), the standard identity (#119)
- `exact_quantum_complexity` used the GF(2) degree in its lower bound; the
  polynomial method requires the real degree (QE ≥ deg(f)/2) (#119)
- Randomized and quantum complexity estimates (R0, R1, R2, Q2, QE) are now
  clamped into certified intervals, e.g. R2 ∈ [bs/3, D] and
  Q2 ∈ [deg̃(f)/2, D] (#119)
