...
class TestFourierKnownCoefficients:
    ...
    def test_dictator_fourier_exact(self):
        ...
        for n in [4, 6, 8]:
            for i in range(min(3, n)):
                ...
                assert len(large_coeffs) == 1, \
                    f"Dictator should have exactly 1 large coefficient"
                assert abs(large_coeffs[0] - 1.0) < 1e-10, \
                    f"Dictator coefficient should be ±1"
    ...
    def test_or_fourier_structure(self):
        ...
        for n in [2, 3, 4, 5]:
            ...
            sum_sq = sum(c**2 for c in fourier)
            assert abs(sum_sq - 1.0) < 1e-10, \
                f"OR_{n}: Parseval failed, sum = {sum_sq}"
...
class TestParseval:
    ...
    def test_parseval_majority(self, n):
        ...
        assert abs(sum_sq - 1.0) < 1e-10, \
            f"Parseval failed for majority_{n}: Σf̂²={sum_sq}"
    ...
    def test_parseval_and(self, n):
        ...
        sum_sq = np.sum(fourier ** 2)
        assert abs(sum_sq - 1.0) < 1e-10, \
            f"Parseval failed for AND_{n}: Σf̂²={sum_sq}"
    ...
    def test_parseval_parity(self, n):
        ...
        sum_sq = np.sum(fourier ** 2)
        assert abs(sum_sq - 1.0) < 1e-10, \
            f"Parseval failed for parity_{n}: Σf̂²={sum_sq}"
        ...
        for i in range(3):
            ...
            for j in range(n):
                if j == i:
                    ...
                    assert abs(influences[j]) < 1e-10, \
                        f"Dictator on x_{i}: Inf_{j} should be 0"
...
class TestTotalInfluence:
    ...
    def test_parity_total_influence_equals_n(self):
        ...
        for n in [3, 5, 7, 9]:
            ...
            assert abs(total - n) < 1e-10, \
                f"Parity_{n}: I[f] should be {n}, got {total}"
    ...
    def test_dictator_total_influence_equals_one(self):
        ...
        for n in [4, 6, 8]:
            ...
            assert abs(total - 1.0) < 1e-10, \
                f"Dictator_{n}: I[f] should be 1, got {total}"
    ...
    def test_and_total_influence(self):
        ...
        for n in [3, 4, 5, 6]:
            ...
            assert abs(total - expected) < 1e-10, \
                f"AND_{n}: I[f] should be {expected}, got {total}"
    ...
    def test_majority_total_influence_asymptotics(self):
        ...
        for n in [11, 13, 15, 17, 19, 21]:
            ...
            assert rel_error < 0.15, \
                f"Majority_{n}: I[f] ≈ {expected_approx:.4f}, got {total:.4f}"
...
class TestNoiseStability:
    ...
    def test_dictator_noise_stability_equals_rho(self, rho):
        ...
        assert abs(stab - rho) < 1e-10, \
            f"Dictator: Stab_{rho} should be {rho}, got {stab}"
    ...
    def test_noise_stability_bounds(self):
        ...
        """
        test_functions = [
            bf.AND(5), bf.OR(5), bf.majority(5), bf.parity(5)
        ]
        ...
        for f in test_functions:
            for rho in [0.3, 0.5, 0.7]:
                ...
                assert -1.01 <= stab <= 1.01, \
                    f"Noise stability should be in [-1, 1], got {stab}"
...
class TestSpectralConcentration:
    ...
    def test_spectral_weight_sums_to_one(self):
        ...
        for f in [bf.AND(5), bf.majority(5), bf.parity(5)]:
            ...
            assert abs(total - 1.0) < 1e-10, \
                f"Spectral weights should sum to 1, got {total}"
    ...
    def test_parity_all_weight_at_top_degree(self):
        ...
        for n in [3, 5, 7]:
            ...
            assert abs(weights.get(n, 0) - 1.0) < 1e-10, \
                f"Parity_{n}: W^{{={n}}} should be 1"
            
            for k in range(n):
                assert abs(weights.get(k, 0)) < 1e-10, \
                    f"Parity_{n}: W^{{={k}}} should be 0"
    ...
    def test_balanced_functions_zero_expectation(self):
        ...
        for f in balanced:
            ...
            assert abs(fourier[0]) < 1e-10, \
                f"Balanced function should have f̂(∅) = 0"
