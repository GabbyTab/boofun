...
import boofun as bf
from boofun.core.representations.bdd import BDDRepresentation, BDD, BDDNode
...
class TestBDDNodeCount:
    ...
    def _count_bdd_nodes(self, node, visited) -> int:
        ...
        return 1 + self._count_bdd_nodes(node.low, visited) + \
               self._count_bdd_nodes(node.high, visited)
    ...
    def test_and_node_count_bound(self):
        ...
        for n in [3, 4, 5]:
            ...
            assert node_count <= n + 2 + 2, \
                f"AND_{n} has {node_count} nodes, expected ≤ {n+4}"
    ...
    def test_or_node_count_bound(self):
        ...
        for n in [3, 4, 5]:
            ...
            assert node_count <= n + 2 + 2, \
                f"OR_{n} has {node_count} nodes, expected ≤ {n+4}"
...
class TestBDDReduction:
    ...
    def test_no_redundant_nodes(self):
        ...
        def check_no_redundant(node, visited=None):
            ...
            return check_no_redundant(node.low, visited) and \
                   check_no_redundant(node.high, visited)
