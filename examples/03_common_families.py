...
for n in [3, 5, 7, 9, 11]:
    ...
    print(f"{n:<5} {expectation(f):<10.3f} {f.total_influence():<10.3f} {theory:<10.3f} {inf_i:<10.4f}")
...
for n in [3, 5, 7, 9]:
    f = bf.parity(n)
    print(f"{n:<5} {f.total_influence():<10.1f} {f.influences()[0]:<10.1f} "
...
print(f"  Theoretical I[f] for n=9: {majority_family.metadata.asymptotics['total_influence'](9):.3f}")
...
for name, f in families:
    print(f"{name:<12} {expectation(f):<8.3f} {f.variance():<8.3f} "
