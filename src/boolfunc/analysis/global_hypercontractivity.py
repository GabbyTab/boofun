...
from typing import TYPE_CHECKING, Dict, Tuple, List, Set, Optional, Callable
import numpy as np
...
class GlobalHypercontractivityAnalyzer:
    ...
    def is_global(self, alpha: float = 0.5, max_set_size: int = 3) -> Tuple[bool, Dict]:
        ...
        if cache_key not in self._globality_cache:
            self._globality_cache[cache_key] = is_alpha_global(
                self.f, alpha, max_set_size, self.p
            )
