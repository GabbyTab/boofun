...
from typing import TYPE_CHECKING, Dict, Any, List, Tuple, Optional, Callable
import numpy as np
...
def pac_learn_low_degree(
    ...
    num_coeffs = sum(
        comb(n, k) for k in range(max_degree + 1)
    )
...
class PACLearner:
    ...
    def learn_low_degree(self, max_degree: int) -> Dict[int, float]:
        ...
        return pac_learn_low_degree(
            self.f, max_degree, self.epsilon, self.delta, self.rng
        )
    
    def learn_junta(self, k: int) -> Tuple[List[int], Dict[int, float]]:
        """Learn assuming k-junta."""
        return pac_learn_junta(
            self.f, k, self.epsilon, self.delta, self.rng
        )
    
    def learn_sparse(self, sparsity: int) -> Dict[int, float]:
        """Learn assuming sparse Fourier spectrum."""
        return pac_learn_sparse_fourier(
            self.f, sparsity, self.epsilon, self.delta, self.rng
        )
    
    def learn_decision_tree(self, max_depth: int) -> Dict[int, float]:
        """Learn assuming decision tree structure."""
        return pac_learn_decision_tree(
            self.f, max_depth, self.epsilon, self.delta, self.rng
        )
    
    def learn_monotone(self) -> Dict[int, float]:
        """Learn assuming monotone function."""
        return pac_learn_monotone(
            self.f, self.epsilon, self.delta, self.rng
        )
