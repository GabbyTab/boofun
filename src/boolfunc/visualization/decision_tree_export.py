...
def build_decision_tree(f: "BooleanFunction", max_depth: Optional[int] = None) -> DecisionTreeNode:
    ...
    def build_subtree(assignment: Dict[int, int], depth: int) -> DecisionTreeNode:
        ...
        if (low_child.is_leaf and high_child.is_leaf and 
            low_child.value == high_child.value):
...
def export_decision_tree_text(
    ...
    def print_tree(node: DecisionTreeNode, prefix: str = "", is_left: bool = True, edge_label: str = ""):
        if node is None:
            return
...
def export_decision_tree_dot(
    ...
    def add_node(node: DecisionTreeNode, parent_id: Optional[int] = None, edge_label: str = "") -> int:
        ...
        if node.is_leaf:
            ...
            lines.append(f'    node{current_id} [label="{label}", shape={shape}, style=filled, fillcolor="{color}"];')
        else:
