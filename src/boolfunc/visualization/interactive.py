    ...
    fig.update_layout(
        polar=dict(
            radialaxis=dict(
                ...
                ticktext=[f'{int(max_val*0.25)}', f'{int(max_val*0.5)}', 
                         f'{int(max_val*0.75)}', f'{int(max_val)}']
            )
        ),
...
def interactive_growth_explorer(
    ...
    fig.update_layout(
        title=f'{family.metadata.name} Family Growth',
        height=height,
        width=width,
        showlegend=False
    )
