...
class PropertyMarker:
    ...
    @staticmethod
    def noise_stability(rho: float = 0.5, theoretical: Optional[Callable[[int, float], float]] = None) -> Marker:
    ...
    def plot(
        ...
        **plot_kwargs,
        ...
        if show_theory and theory_arr is not None and not np.any(np.isnan(theory_arr)):
            ax.plot(n_arr, theory_arr, '--', color='gray', 
                   label=f"{marker_name} (theory)", alpha=0.7)
