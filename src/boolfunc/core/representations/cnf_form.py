...
class CNFClause:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'CNFClause':
        ...
        return cls(
            positive_vars=set(data["positive_vars"]),
            negative_vars=set(data["negative_vars"])
        )
...
class CNFFormula:
    ...
    def simplify(self) -> 'CNFFormula':
        ...
        for clause in self.clauses:
            ...
            if not is_redundant:
                ...
                simplified_clauses = [c for c in simplified_clauses 
                                    if not self._subsumes(clause, c)]
    ...
    def _subsumes(self, clause1: CNFClause, clause2: CNFClause) -> bool:
        ...
        """
        return (clause1.positive_vars <= clause2.positive_vars and
                clause1.negative_vars <= clause2.negative_vars)
