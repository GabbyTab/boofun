...
class LTFParameters:
    ...
    def __post_init__(self):
        ...
        if len(self.weights) != self.n_vars:
            raise ValueError(f"Weights length {len(self.weights)} doesn't match n_vars {self.n_vars}")
...
class LTFRepresentation(BooleanFunctionRepresentation[LTFParameters]):
    ...
    def is_complete(self, data: LTFParameters) -> bool:
        ...
        return (data.weights is not None and 
                data.threshold is not None and 
                len(data.weights) == data.n_vars)
