...
BooleanFunc.py with the modern boolfunc library, enabling:
...
Example usage:
    >>> from boolfunc.core.legacy_adapter import from_legacy, to_legacy
    >>> 
...
def is_legacy_object(obj: Any) -> bool:
    ...
    return (
        hasattr(obj, 'f') and 
        hasattr(obj, 'k') and 
        hasattr(obj, 'fix_single') and
        hasattr(obj, 'FourierTransform')
    )


def from_legacy(
    legacy_func: Any,
    import_fourier: bool = False
) -> "BooleanFunction":
    """
    ...
    new_func = BooleanFunctionFactory.from_truth_table(
        BooleanFunction, 
        truth_table, 
        n=n_vars
    )
...
class LegacyWrapper:
    ...
    def __init__(self, func: "BooleanFunction"):
        ...
        """
        self._func = func
        self._f = None  # Cached truth table list
...
def convert_legacy_function(
    ...
    return BooleanFunctionFactory.from_truth_table(
        modern_class,
        np.array(truth_table, dtype=bool),
        n=n_vars
    )
