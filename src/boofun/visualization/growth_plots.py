...
if TYPE_CHECKING:
    ...
    from ..families.tracker import GrowthTracker, TrackingResult
...
class GrowthVisualizer:
        ...
        **kwargs
        ...
        if self.backend == "matplotlib":
            return self._plot_growth_matplotlib(
                n_arr, computed_arr, theory_arr,
                marker_name, tracker.family.metadata.name,
                show_theory, log_x, log_y, figsize, title, ax, **kwargs
            )
        else:
            return self._plot_growth_plotly(
                n_arr, computed_arr, theory_arr,
                marker_name, tracker.family.metadata.name,
                show_theory, log_x, log_y, title, **kwargs
            )
    
    def _plot_growth_matplotlib(
        self, n_arr, computed_arr, theory_arr,
        marker_name, family_name, show_theory, log_x, log_y,
        figsize, title, ax, **kwargs
    ):
        ...
        ax.plot(n_arr, computed_arr, 'o-', color=color, 
               linewidth=2, markersize=8, label=f'{marker_name} (computed)')
        
        # Plot theoretical values
        if show_theory and theory_arr is not None:
            valid_theory = ~np.isnan(theory_arr.astype(float))
            if np.any(valid_theory):
                ax.plot(n_arr[valid_theory], theory_arr[valid_theory], 
    ...
    def _plot_growth_plotly(
        self, n_arr, computed_arr, theory_arr,
        marker_name, family_name, show_theory, log_x, log_y, title, **kwargs
    ):
        ...
        if show_theory and theory_arr is not None:
            ...
            if np.any(valid_theory):
                fig.add_trace(go.Scatter(
                    x=n_arr[valid_theory], y=theory_arr[valid_theory],
                    mode='lines',
    ...
    def _compare_families_matplotlib(
        ...
        for i, (name, tracker) in enumerate(trackers.items()):
            ...
            if show_theory and theory_arr is not None:
                ...
                if np.any(valid):
                    ax.plot(n_arr[valid], theory_arr[valid], '--',
    ...
    def _compare_families_plotly(
        self, trackers, marker_name, show_theory, log_x, log_y, title
    ):
        ...
        for i, (name, tracker) in enumerate(trackers.items()):
            ...
            if show_theory and theory_arr is not None:
                ...
                if np.any(valid):
                    fig.add_trace(go.Scatter(
                        x=n_arr[valid], y=theory_arr[valid],
                        mode='lines',
                        name=f'{name} (theory)',
    ...
    def plot_convergence_rate(
        ...
        if self.backend == "matplotlib":
            ...
            ax.plot(n_arr, ratio, 'o-', linewidth=2, markersize=8,
                   color=self.colors[0])
            
            # Fit a horizontal line (constant) if converging
            if len(ratio) > 2:
                mean_ratio = np.mean(ratio[-3:])  # Last 3 points
                ax.axhline(y=mean_ratio, color='red', linestyle='--',
                          alpha=0.7, label=f'Limiting value ≈ {mean_ratio:.4f}')
            ...
            fig.add_trace(go.Scatter(
                x=n_arr, y=ratio,
                mode='lines+markers',
                name=f'{marker_name} / {ref_label}'
            ))
            
            if len(ratio) > 2:
                mean_ratio = np.mean(ratio[-3:])
                fig.add_hline(y=mean_ratio, line_dash='dash',
                            annotation_text=f'≈ {mean_ratio:.4f}')
    ...
    def plot_multi_property_growth(
        ...
        if self.backend == "matplotlib":
            ...
            for ax, marker_name in zip(axes, marker_names):
                self.plot_growth(tracker, marker_name, ax=ax, 
                               show_theory=True, title=marker_name)
            
            fig.suptitle(f'{tracker.family.metadata.name} Family Properties',
                        fontsize=14)
            plt.tight_layout()
            return fig
            ...
            fig = make_subplots(
                rows=1, cols=n_markers,
                subplot_titles=marker_names
            )
            ...
            for i, marker_name in enumerate(marker_names, 1):
                if marker_name in tracker.results:
                    ...
                    fig.add_trace(
                        go.Scatter(x=n_arr, y=computed_arr, 
                                  mode='lines+markers', name=marker_name),
                        row=1, col=i
                    )
            
            fig.update_layout(
                title=f'{tracker.family.metadata.name} Family Properties',
                showlegend=False
            )
...
class LTFVisualizer:
    ...
    def plot_weight_distribution(
        ...
        ax2.axhline(y=0.5, color='red', linestyle='--', alpha=0.7, 
                   label='Critical index threshold')
    ...
    def plot_influence_vs_weight(
        ...
        from ..analysis.ltf_analysis import is_ltf, find_ltf_weights
        ...
        for i, (w, inf) in enumerate(zip(np.abs(weights), influences)):
            ax.annotate(f'x_{i}', (w, inf), xytext=(5, 5), 
...
def quick_growth_plot(
    ...
    **kwargs
    ...
    from ..families import (
        ...
        ANDFamily, ORFamily, DictatorFamily, GrowthTracker
    )
