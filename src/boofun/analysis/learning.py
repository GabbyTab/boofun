...
def _gl_tree_search(
    ...
    while queue:
        ...
        if depth == n:
            ...
            est0, _ = _estimate_subtree_weight(f, new_mask, prefix_value, threshold, num_samples, rng)
            if est0 >= threshold ** 2 / 4:  # Could contain heavy coefficient
                queue.append((new_mask, prefix_value))
            
            # Branch where variable is 1 (in S)
            est1, _ = _estimate_subtree_weight(f, new_mask, prefix_value | (1 << next_var), threshold, num_samples, rng)
