...
from ..utils.exceptions import BooleanFunctionError, InvalidInputError
...
def load_json(path: Union[str, Path], **kwargs) -> "BooleanFunction":
    ...
    if rep_type == "truth_table":
        ...
        return BooleanFunctionFactory.from_truth_table(
            BooleanFunction, values, n=n_vars
        )
    elif rep_type == "fourier_expansion":
        coeffs = np.array(data.get("coefficients", []))
        return BooleanFunctionFactory.from_multilinear(
            BooleanFunction, coeffs, n=n_vars
        )
    elif rep_type == "anf":
        ...
        return BooleanFunctionFactory.from_polynomial(
            BooleanFunction, monomials, n=n_vars
        )
    elif rep_type == "dnf":
...
def save_json(
    ...
    **kwargs,
    ...
    export_data = strategy.dump(data, n_vars=func.n_vars)
...
def load_bf(path: Union[str, Path], **kwargs) -> "BooleanFunction":
    ...
    # Create the function
    func = BooleanFunctionFactory.from_truth_table(
        BooleanFunction, truth_table, n=n_vars
    )
    ...
    with open(path, "r") as f:
        for line in f:
            ...
            if line.startswith("p cnf"):
                ...
                if len(parts) >= 4:
                    n_vars = int(parts[2])
                    n_clauses_expected = int(parts[3])
                continue
    ...
    with open(path, "w") as f:
        ...
        if comment:
            for line in comment.split("\n"):
                f.write(f"c {line}\n")
        f.write(f"c Generated by BooFun\n")
