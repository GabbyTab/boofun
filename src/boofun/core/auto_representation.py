...
def recommend_representation(
    ...
    if sparsity is not None and sparsity < SPARSE_RATIO_THRESHOLD:
        ...
        recommendation["memory_mb"] = f"~{sparsity * size * 12 / (1024 * 1024):.1f} (depends on actual)"
        return recommendation
    
    # Medium n: use packed
    if n_vars <= PACKED_THRESHOLD:
        recommendation["representation"] = "packed_truth_table"
        recommendation["reason"] = f"{DENSE_THRESHOLD} < n={n_vars} <= {PACKED_THRESHOLD}: packed saves memory"
