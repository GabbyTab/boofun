...
| Influences match BoolForge exact activities; total influence matches BoolForge exact average sensitivity | BoolForge v1.0.1⁵ | majority(5) | float, rtol 1e-2 (fp noise only; `exact=True`, no sampling) | [`test_boolforge.py::TestBoolForgeSpectral`](https://github.com/GabbyTab/boofun/blob/main/tests/cross_validation/test_boolforge.py) |
