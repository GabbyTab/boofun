...
def capitalize(s):
    if "." in s:
        s = ".".join(map(capitalize_word, s.split(".")[:-1])) + "." + s.split(".")[-1]
    for delim in [" ", "_", "-", ":"]:
...
Chernoff = lambda x, y, base=math.e: x * math.log(x / y, base) + (1 - x) * math.log(
    (1 - x) / (1 - y), base
)
...
def search_min(f, from_, to, iter=50):
    for i in range(iter):
        points = [from_ + ((to - from_) * i / 4.0) for i in range(5)]
...
entinv = lambda x: search_zero(lambda p: ent(p) - x, 0.0, 0.5)
